...
import Style from  './Post_CM.module.css'
import StyleGeral from '../cssgeral.module.css'
...
const Post = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        const NovaCM = {
            cod_maquina, descricao, comprimento, largura, pecas_hora, custo_reposicao, amortizacao, cust_maquina, energia_gas, insumo, total_maq, total_maq_anterior, qtd_operadores, mod_encargos, cust_maquina, total_mod, total_mod_anterior,
            eficiencia_maq, eficiencia_ope, eficiencia_pro, taxa_pro
        };

        const msg = await axios.post('http://localhost:3333', NovaCM)

        if (msg.message !== 201) {
            setErro(msg.data.message)
        }
    ...
    return (
        <div div className={StyleGeral.body}>

            <SideBar />

            <div className={StyleGeral.conteiner}>

            <div className={Style.CustoMaquina}>
                <h1>Custo Maquina</h1>
